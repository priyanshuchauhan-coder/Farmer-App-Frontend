...
export const EcommProvider = ({ children }) => {
  ...
  const API_BASE_URL = `${apiUrl}/api/market`;
  

  // Prediction Input state
  const [toSellCrops,setToSellCrops]=useState([]);
...
//   const [fieldErrors, setFieldErrors] = useState({});
  ...
  const getAllToSellCrops = useCallback(async () => {
    try {
...
if (rdata?.status === 404) {
    return { success: false, error: 'No prediction input data found for this farmer.' };
      ...
      if (rdata?.success) {
        showAlert('Crops Fetched Successfully', 'success');
        console.log(rdata);
        setToSellCrops(rdata?.data)
        ...
        showAlert(rdata?.error || 'Crops Not Fetched ', 'danger');
  ...
  const saveCropData = useCallback(async (d) => {
  try {
      ...
      if (rdata?.success) {
        showAlert('Crop Data Submitted Successfully', 'success');
        console.log(rdata);
 ...
 const getCropData = useCallback(async (inputId) => {
  try {
      ...
      if (rdata?.success) {
        showAlert('Crop Data Fetched Successfully', 'success');
        console.log(rdata);
        ...
        showAlert('Crop Data Not Fetched Successfully', 'danger');
      return {success:false};
      }
    } catch (err) {
      showAlert(err.message || 'Something Went Wrong', 'danger');
  ...
  const updateSellCrop = useCallback(async (inputId,d) => {
    try {
      const rdata = await makeRequest({
        url: `/crops/${inputId}`,
        method: 'put',
      ...
      if (rdata?.success) {
        showAlert('Crop Data Updated Successfully', 'success');
        console.log(rdata);
        
        
        
      } else {
        showAlert(rdata?.error || 'Crop Data Not Updated  ', 'danger');
        return {success:false}
      }
      return {success:true,data:rdata?.data};
    } catch (err) {
  ...
  const deleteSellCrop = useCallback(async (inputId) => {
    try {
      const rdata = await makeRequest({
       url: `/crops/${inputId}`,
        method: 'delete',
      ...
      if (rdata?.success) {
        showAlert('Crop Data Deleted Successfully', 'success');
        console.log(rdata);
        // setPredictionInputs(rdata?.data)
        
        
      } else {
        showAlert(rdata?.error || 'Crop Data Not Deleted  ', 'danger');
        return {success:false}
      }
      return {success:true,data:rdata?.data};
    } catch (err) {
  ...
  try {
    const cropsResult = await getAllToSellCrops();
...
// }
    ...
    const inputs = cropsResult.data;
    setToSellCrops(inputs);
